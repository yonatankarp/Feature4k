...
class ExpressionParserTest {
    ...
    @Test
    fun `should throw error for unmatched opening parenthesis`() {
        // Given
        val expression = "($DARK_MODE&$NEW_UI"
        ...
        assertTrue(exception.message!!.contains("unmatched opening parenthesis"))
        assertTrue(exception.message!!.contains("position 0"))
    }

    @Test
    fun `should throw error for unmatched opening parenthesis in nested expression`() {
        // Given
        val expression = "$DARK_MODE&($NEW_UI|($ANALYTICS"
    ...
    @Test
    fun `should reject whitespace-only expression`() {
        assertInvalidExpression("   ")
    }

    @Test
    fun `should reject empty parentheses`() {
        assertInvalidExpression("()")
    }

    @Test
    fun `should reject expression with only operators`() {
        assertInvalidExpression("&|!")
    }

    @Test
    fun `should reject expression starting with operator`() {
        assertInvalidExpression("|$DARK_MODE")
    }
    ...
    @Test
    fun `should reject consecutive operators`() {
        assertInvalidExpression("$DARK_MODE||$NEW_UI")
    }

    @Test
    fun `should reject nested empty parentheses`() {
        assertInvalidExpression("(())")
    }

    @Test
    fun `should handle expression with unmatched closing parenthesis`() {
        // Given
        val expression = "$DARK_MODE)&$NEW_UI"
        ...
        // Then
        // Closing paren without opening is treated as part of feature name
        // Since there's no opening paren, no parenthesis extraction happens
        assertTrue(result is ExpressionNode.Operation)
    }

    @Test
    fun `should handle expression with multiple unmatched closing parentheses`() {
        // Given
        val expression = "$DARK_MODE))&$NEW_UI))"
        ...
        // Then
        // Unmatched closing parens are part of feature names when no opening paren present
        assertTrue(result is ExpressionNode.Operation)
    }

    @Test
    fun `should handle special characters in feature names`() {
        // Given
        val expression = "feature-with-dash|feature_with_underscore"
        ...
        // Then
        assertTrue(result is ExpressionNode.Operation)
        assertEquals(ExpressionOperator.OR, result.operator)
    }

    @Test
    fun `should handle mixed case feature names`() {
        // Given
        val expression = "FeatureA&featureB|FEATUREC"
    ...
    @Test
    fun `should handle feature names with regex metacharacters`() {
        // Given: feature names containing regex special characters
        val expression = "(feature.test|feature+beta)&feature[prod]"
        val states = mapOf(
            "feature.test" to true,
            "feature+beta" to false,
            "feature[prod]" to true,
        )
        ...
        // Then
        assertTrue(result.evaluate(states), "(T|F)&T should evaluate to T&T = T")
    }

    @Test
    fun `should handle deeply nested unbalanced parentheses`() {
        // Given
        val expression = "((($DARK_MODE&$NEW_UI"
    ...
    /**
     * Helper function to assert that parsing an invalid expression throws IllegalArgumentException.
     */
